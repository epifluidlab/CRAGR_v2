...
def combine_samples_to_reps(file_list, output_file, chroms_list):
	temp_output = output_file.rstrip(".sorted.gz")
	with open(temp_output, 'wb') as out_f:
		for file in file_list:
			with subprocess.Popen(["bgzip", "-c", file], stdout=subprocess.PIPE) as proc:
	...
	subprocess.run(["tabix", "-p bed", sorted_output + ".gz"])


rule all:
    input:
        expand(out_dir + "/rep{rep}.frag.sorted.gz.tbi", rep=[1, 2])
...
rule split_files:
    input:
        samples_list
    output:
        rep1 = out_dir + "/rep1_samples.txt",
        rep2 = out_dir + "/rep2_samples.txt"
    run:
        rep1_samples, rep2_samples = split_files_to_reps(input.samples_list, seed=seed)
...
rule combine_samples:
    input:
        rep1_samples = out_dir + "/rep1_samples.txt",
        rep2_samples = out_dir + "/rep2_samples.txt"
    output:
        rep1_output = out_dir + "/rep1.frag.sorted.gz",
        rep2_output = out_dir + "/rep2.frag.sorted.gz"
    run:
